...
async fn main() -> Result<()> {
    ...
    let mut config_path = exe_dir.join("config").join("ytmusic.json");
...
print_help();
    ...
    mpv_cmd(&socket, json!({"command":["set_property","loop-file","inf"]}))?;
                ...
                if k.kind == KeyEventKind::Press {
                    match k.code {
                        ...
                        KeyCode::Char('a')  => {
                            auto_next = !auto_next;
                            if auto_next {
                                println!("Auto-play mode ON (continuous playback)");
                                mpv_cmd(&socket, json!({"command":["set_property","loop-file","no"]}))?;
                            } else {
                                println!("Loop mode ON (repeat current track)");
                                mpv_cmd(&socket, json!({"command":["set_property","loop-file","inf"]}))?;
                            }
                        }
                        KeyCode::Char('n')  => {
...
fn print_help() {
    ...
    println!("r  Restart   a  Toggle Auto-play");
    println!("q  Quit");
    println!("Default: Loop current track | Press 'a' for continuous playback");
}
