...
- Single track loop or continuous playback mode
...
```bash
# Search and play
TY-mushell.exe "涙そうそう"
TY-mushell.exe "再會啦！心愛的無緣的人"
TY-mushell.exe "hamilton-musical"
```
...
| `a` | Toggle Auto-play (Loop ⇄ Continuous) |
| `q` | Quit |

**Default mode:** Loop current track (single repeat)  
**Press 'a':** Switch to continuous playback (auto next)

## Installation

### Option 1: Download Release

1. Download the latest release from [Releases](https://github.com/2460124601/YT-Mushell/releases)
2. Extract to any folder
3. **Download MPV** from https://github.com/zhongfly/mpv-winbuild/releases/download/2025-10-31-d3ec15b/mpv-x86_64-20251031-git-d3ec15b.7z
   - Direct link: `mpv-x86_64-20251031-git-d3ec15b.7z` (for 64-bit system)
   - Extract all files to `third_party\mpv\` folder
4. Configure `config/ytmusic.json` (see Configuration)
5. Run `YT-mushell.exe "your query"`

### Option 2: Build from Source

**Prerequisites:**
- Rust (install from https://rustup.rs)
- MPV Player (download from https://github.com/zhongfly/mpv-winbuild/releases)

```bash
# Clone repository
git clone https://github.com/2460124601/YT-Mushell.git
cd YT-Mushell

# Download and extract MPV to third_party/mpv/
# From: https://github.com/zhongfly/mpv-winbuild/releases

# Build release
cargo build --release

# Run
.\target\release\YT-mushell.exe "song name"
```

**Note:** MPV binaries (~100MB) are not included in the repository to keep it lightweight. Please download manually from the link above.
...
Copy-Item "target\release\YT-mushell.exe" "release-package\" -Force
Copy-Item "README.md" "release-package\" -Force
Copy-Item "get-ytmusic-config.js" "release-package\" -Force
Copy-Item -Recurse "config" "release-package\" -Force
Copy-Item -Recurse "third_party" "release-package\" -Force

# Create zip
Compress-Archive -Path release-package\* -DestinationPath YT-mushell-windows.zip
```
